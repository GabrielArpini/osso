...
class NaiveKVCache(BaseKVCache):
    def __init__(self, config: ModelConfig, device: torch.device, dtype: torch.dtype, max_seq_len: int | None = None) -> None:
        seq_len = max_seq_len if max_seq_len is not None else config.max_seq_len
        self._kv_buffer = torch.zeros(
            (2, config.n_layers, config.batch_size, seq_len, config.num_kv_heads, config.head_dim),
            device=device,
            dtype=dtype,
        )
    ...
    def k_cache(self, layer_id: int) -> torch.Tensor:
        return self._k_buffer[layer_id][:, :self._cache_end]

    def v_cache(self, layer_id: int) -> torch.Tensor:
        return self._v_buffer[layer_id][:, :self._cache_end]
    ...
    def store(self, layer_id: int, start_pos: int, k: torch.Tensor, v: torch.Tensor) -> None:
        ...
        self._cache_end = start_pos + new_len
